...
| Verificación realizada | Revisión estática línea por línea del script contra `TP1_FoodStore/schema.sql` (columnas y tipos reales). La corrida real dentro de `BEGIN ... ROLLBACK` con `INSERT` válidos e inválidos queda pendiente de ejecutar en un entorno con PostgreSQL disponible, según el protocolo de seguridad de la Parte 0. |
